...
exports.demoUser = exports.database = exports.availableServices = exports.app = void 0;
...
const encodePassword = (password) => Buffer.from(password).toString('base64');
const getCurrentUser = (req) => req.user;
const createNotification = (userId, title, message, type = 'info', actionUrl) => ({
    id: (0, uuid_1.v4)(),
    userId,
    title,
    message,
...
const database = {
    ...
    notifications: [
        createNotification(demoUser.id, 'Bienvenue sur AREA', 'Votre compte demo est pret avec une integration GitHub active.', 'success'),
        createNotification(demoUser.id, 'Workflow surveille', 'Le scenario GitHub vers Discord est actif et en attente d evenements.', 'info', '/workflows')
    ]
};
...
app.post('/api/auth/register', (req, res) => {
    ...
    database.notifications.unshift(createNotification(user.id, 'Compte cree', 'Votre espace AREA est initialise.', 'success'), createNotification(user.id, 'Prochaine etape', 'Connectez un service pour lancer vos automatisations.', 'info'));
    res.status(201).json({
        success: true,
        data: {
...
app.get('/api/services', authMiddleware, (req, res) => {
    ...
    const connectedIds = new Set(database.connectedServices.filter((service) => service.userId === user.id).map((service) => service.serviceId));
    res.json({
        success: true,
...
app.post('/api/services/:id/connect', authMiddleware, (req, res) => {
    ...
    const existing = database.connectedServices.find((entry) => entry.userId === user.id && entry.serviceId === service.id);
    if (existing) {
    ...
    const connectedService = {
        ...
        accountName: req.body.accountName || `${service.name} Workspace`,
        accessToken: (0, uuid_1.v4)(),
        isActive: true,
        connectedAt: new Date().toISOString()
    };
    database.connectedServices.push(connectedService);
    database.notifications.unshift(createNotification(user.id, `${service.name} connecte`, `${service.name} est pret a alimenter vos workflows.`, 'success', '/services'));
    res.status(201).json({ success: true, data: connectedService });
});
app.delete('/api/services/:id/disconnect', authMiddleware, (req, res) => {
    const user = getCurrentUser(req);
    const index = database.connectedServices.findIndex((entry) => entry.userId === user.id && entry.serviceId === req.params.id);
    if (index === -1) {
        res.status(404).json({ error: 'Connected service not found' });
        return;
    }
    const [removed] = database.connectedServices.splice(index, 1);
    database.notifications.unshift(createNotification(user.id, 'Service deconnecte', `${removed.serviceId} a ete retire de vos integrations.`, 'warning', '/services'));
...
app.post('/api/workflows', authMiddleware, (req, res) => {
    ...
    database.notifications.unshift(createNotification(user.id, 'Workflow cree', `${workflow.name} est enregistre et en attente de signaux.`, 'success', '/workflows'));
...
app.put('/api/workflows/:id', authMiddleware, (req, res) => {
    ...
    database.notifications.unshift(createNotification(user.id, workflow.isActive ? 'Workflow actif' : 'Workflow en pause', `${workflow.name} a ete mis a jour.`, workflow.isActive ? 'info' : 'warning', '/workflows'));
...
app.delete('/api/workflows/:id', authMiddleware, (req, res) => {
    ...
    database.notifications.unshift(createNotification(user.id, 'Workflow supprime', `${removed.name} a ete supprime de votre espace.`, 'warning', '/workflows'));
...
app.put('/api/users/me', authMiddleware, (req, res) => {
    ...
    database.notifications.unshift(createNotification(user.id, 'Profil mis a jour', 'Vos informations ont ete sauvegardees.', 'success', '/profile'));
